...
def model_to_dict(instance, field_list, date_strf=None):
    ...
    def check_field_list(obj):
        """Verify that obj is a list of strings"""
        return bool(obj) and all(isinstance(elem, str) for elem in obj)

    if not check_field_list(field_list):
        raise IncorrectInputTypeException("field_list argument must be a list or tuple of fields")
    ...
    for f in chain(opts.concrete_fields, opts.private_fields, opts.many_to_many):
        ...
        if isinstance(f, DateTimeField):
            ...
            if instance.pk is None:
                ...
                if qs._result_cache is not None:
                    ...
                    try:
                        m2m_field = list(filter(lambda a: f.name in a and a.find("__") != -1, field_list))[0]
        ...
        # ToDo: Process other types of model fields that might have been provided in field_list
